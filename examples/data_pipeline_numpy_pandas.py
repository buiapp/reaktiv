...
def main():
    ...
    with_zscore = Computed(lambda: 
        basic_stats().assign(
            temp_zscore=(basic_stats()['temperature'] - basic_stats()['temp_mean']) / basic_stats()['temp_std']
        )
    )
    
    # Stage 3: Outlier detection - depends on with_zscore AND threshold
    with_outliers = Computed(lambda: 
        with_zscore().assign(
    ...
    def print_summary():
        ...
        print(f"Mean: {s['mean_temp']:.1f}°C, Outliers: {s['outliers']}, Sensors: {s['sensor_counts']}")
    
    report = Effect(print_summary)
    
    # Demonstrate fine-grained reactive updates
    print("Changing window size (affects basic_stats, with_zscore, with_outliers, summary)...")
    window_size.set(20)  
