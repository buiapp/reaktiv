...
class IoTSensorAgent:
    ...
    def __init__(self):
        ...
        self.sensor_status = Computed(lambda: 
            "ERROR" if self.sensor_error() else
            "ACTIVE" if self.is_running() else "STANDBY"
        )
    ...
    def _sensor_loop(self):
        ...
        try:
            while self.is_running():
                ...
                new_temp = max(10, min(35, self.temperature() + random.uniform(-0.5, 0.5)))
...
def demo():
    ...
    log_sensor_eff = Effect(log_sensor)
    temp_alert_eff = Effect(temp_alert)
    monitor_status_eff = Effect(monitor_status)
    control_climate_eff = Effect(control_climate)
    
    try:
        print("Starting IoT sensor monitoring system...")
        print("All monitoring, alerts, and climate control will react automatically to sensor changes")
        sensor.start_sensor()
