...
class TodoState:
    def __init__(self):
        ...
        self.active_count = Computed(
            lambda: sum(1 for todo in self.todos() if not todo["completed"])
        )
        self.completed_count = Computed(
            lambda: sum(1 for todo in self.todos() if todo["completed"])
        )
    ...
    def clear_completed(self):
        self.todos.update(
            lambda todos: [todo for todo in todos if not todo["completed"]]
        )
...
with ui.card():
    ...
    with ui.row():
        ...
        ui.button(
            "Add",
            on_click=lambda: [state.add_todo(new_todo.value), new_todo.set_value("")],
        )
            ...
            with todo_container:
                with ui.row():
                    ui.checkbox(
                        ...
                        on_change=lambda e, idx=i: state.toggle_todo(idx),
                    )
