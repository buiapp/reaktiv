...
class ReactiveMatrixVisualizer:
    def __init__(self):
        ...
        self.gs = gridspec.GridSpec(2, 2, height_ratios=[6, 1], width_ratios=[20, 1], 
                                   figure=self.fig, hspace=0.05, wspace=0.05)
        ...
        self.status_text = self.fig.text(0.05, 0.25, "", fontsize=10, 
                                        bbox=dict(facecolor='yellow', alpha=0.2))
        
        # Rectangle for highlighting updates
        self.highlight_rect = Rectangle((0, 0), 1, 1, fill=False, 
                                      edgecolor='red', linewidth=3, visible=False)
        ...
        self.input_parameters = Computed(lambda: {
        ...
        self.filtered_matrix = Computed(lambda: self.filtered_matrix_with_time()["filtered"])
        ...
        if filter_size <= 1:
            ...
            for i in range(m):
                for j in range(n):
                    ...
                    filtered[i, j] = center_val * (1 - effect_strength) + np.mean(window) * effect_strength
    ...
    def _track_matrix_computation(self):
        ...
        self.status_message.set(f"Matrix {params['size']}×{params['size']} with seed {params['seed']}")
    ...
    def _update_plot(self):
        ...
        im = self.ax.imshow(matrix, cmap=cmap_name, interpolation='nearest',
        ...
        if show_contours:
            contours = self.ax.contour(matrix, levels=contour_levels, 
                                     colors='k', alpha=0.7, linewidths=1.5)
    ...
    def _setup_controls(self):
        ...
        ax_seed = plt.axes([0.25, 0.05, 0.65, 0.03], facecolor=axcolor)  # Fixed: added missing height parameter
        
        # Store sliders as instance variables
        self.size_slider = Slider(ax_size, 'Matrix Size', 50, 500, valinit=self.matrix_size(), valstep=10)
        self.noise_slider = Slider(ax_noise, 'Noise Scale', 0.1, 2.0, valinit=self.noise_scale())
        self.filter_slider = Slider(ax_filter, 'Filter Size', 0, 10, valinit=self.filter_size(), valstep=1)
        self.seed_slider = Slider(ax_seed, 'Random Seed', 0, 100, valinit=self.seed(), valstep=1)
        ...
        ax_contour = plt.axes([0.15 + button_width + button_spacing, button_y, button_width, 0.03], facecolor=axcolor)
        ax_contour_levels = plt.axes([0.15 + 2*(button_width + button_spacing), button_y, button_width, 0.03], facecolor=axcolor)
        ax_reset = plt.axes([0.15 + 3*(button_width + button_spacing), button_y, button_width, 0.03], facecolor=axcolor)
...
if __name__ == "__main__":
    ...
    print("Notice how expensive computations only run when computation parameters change,")
    print("while visualization settings can be changed without triggering recalculation.")
