...
class TestBatchPerformance:
    ...
    async def test_no_unnecessary_computations_in_batch(self):
        ...
        compute_independent_mock = Mock(side_effect=lambda: 10)  # Independent of a, b, c
        ...
        with batch():
            a.set(5)  # This should only affect computed_ab, not computed_bc or computed_independent
        ...
        assert compute_ab_mock.call_count == 1, "computed_ab should recompute once (dependency 'a' changed)"
        assert compute_bc_mock.call_count == 0, "computed_bc should NOT recompute (no dependencies changed)"
        assert compute_independent_mock.call_count == 0, "computed_independent should NOT recompute (no dependencies changed)"
    ...
    async def test_no_unnecessary_effect_retriggers_in_batch(self):
        ...
        assert effect_xy_mock.call_count == 1, "effect_xy should run once (depends on x via computed_xy)"
        assert effect_z_mock.call_count == 0, "effect_z should NOT run (z didn't change)"
        assert effect_all_mock.call_count == 1, "effect_all should run once (depends on x via computed_xy)"
    ...
    async def test_no_duplicate_notifications_in_complex_batch(self):
        ...
        combined = ComputeSignal(lambda: left.get() + right.get())  # depends on both left and right
        ...
        assert effect_mock.call_count == 1, "Effect should run exactly once, not multiple times for diamond dependency"
    ...
    async def test_respect_custom_equality_in_batch(self):
        ...
        assert effect_mock.call_count == 0, "Effect should not run when computed value is considered equal by custom equality"
        ...
        assert effect_mock.call_count == 1, "Effect should run when computed value changes beyond equality threshold"
    ...
    async def test_no_computation_when_dependencies_unchanged_in_batch(self):
        ...
        assert stable_compute_mock.call_count == 0, "stable_computed should NOT recompute (dependency unchanged)"
        assert changing_compute_mock.call_count == 1, "changing_computed should recompute once (dependency changed)"
    ...
    async def test_forced_recomputation_issue_in_deferred_processing(self):
        ...
        try:
            ...
            stable_mock = Mock(side_effect=lambda: b.get() + 100)   # Depends on 'b' (won't change)
            ...
            print(f"After creation - changing_computed subscribers: {len(changing_computed._subscribers)}")
            print(f"After creation - stable_computed subscribers: {len(stable_computed._subscribers)}")
            print(f"After creation - independent_computed subscribers: {len(independent_computed._subscribers)}")
            ...
            print(f"After effects created - changing_computed subscribers: {len(changing_computed._subscribers)}")
            print(f"After effects created - stable_computed subscribers: {len(stable_computed._subscribers)}")  
            print(f"After effects created - independent_computed subscribers: {len(independent_computed._subscribers)}")
            ...
            print(f"Before batch - independent_computed dirty: {independent_computed._dirty}")
            ...
            with batch():
                ...
                print(f"After a.set(5) - changing_computed dirty: {changing_computed._dirty}")
                print(f"After a.set(5) - stable_computed dirty: {stable_computed._dirty}")
                print(f"After a.set(5) - independent_computed dirty: {independent_computed._dirty}")
            ...
            print(f"After batch - independent_computed dirty: {independent_computed._dirty}")
            ...
            assert changing_mock.call_count == 1, "changing_computed should recompute (dependency changed)"
            ...
            if stable_recomputed:
                print(f"⚠️  PERFORMANCE ISSUE: stable_computed recomputed {stable_mock.call_count} times unnecessarily")
            if independent_recomputed:
                print(f"⚠️  PERFORMANCE ISSUE: independent_computed recomputed {independent_mock.call_count} times unnecessarily")
                
            # The fix should ensure these don't recompute unnecessarily
            assert stable_mock.call_count == 0, "stable_computed should NOT recompute (dependencies unchanged)"
            assert independent_mock.call_count == 0, "independent_computed should NOT recompute (no dependencies on changed signals)"
