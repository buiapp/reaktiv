...
async def test_async_effect_trigger_count():
    ...
    # Assert
    assert initial_count == 1, "Async effect should be triggered once during initialization"
    assert after_update_count == 2, "Async effect should be triggered only once more after signal update"
...
async def test_async_complex_dependency_chain():
    ...
    # Initial state
    assert initial_trigger_count == 1, "Async effect should be triggered once during initialization"
    ...
    # The effect should only be triggered once more
    assert after_update_count == 2, "Async effect should be triggered exactly once after signal update"
...
async def test_async_batch_update_effect_trigger():
    ...
    await asyncio.sleep(0)  # Wait for async effect to complete
    final_count = trigger_count
    assert final_count == 2, "Async effect should trigger exactly once after the batch update"
...
async def test_async_effect_batching_stress_test():
    ...
    assert after_update_count == 2, "Async effect should trigger exactly once after signal update, regardless of how many computed signals depend on it"
...
async def test_mixed_sync_and_async_effects():
    ...
    assert after_sync_count == 2, "Sync effect should trigger exactly once after signal update"
    assert after_async_count == 2, "Async effect should trigger exactly once after signal update"
